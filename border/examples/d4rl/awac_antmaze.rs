...
use border_minari::{
    d4rl::antmaze::candle::{AntMazeConverter, AntMazeConverterConfig},
...
const MODEL_DIR: &str = "border/examples/d4rl/model/candle/awac_antmaze";
const MLFLOW_EXPERIMENT_NAME: &str = "D4RL";
const MLFLOW_TAGS: &[(&str, &str)] = &[("algo", "awac"), ("backend", "candle")];

/// Train AWAC agent in antmaze environment
...
struct Args {
    ...
    /// https://minari.farama.org/v0.5.1/datasets/D4RL/antmaze/
    #[arg(long)]
    env: String,
...
impl Args {
    pub fn env_name(&self) -> String {
        format!("antmaze/{}", self.env)
    }

    pub fn dataset_name(&self) -> String {
        format!("D4RL/antmaze/{}", self.env)
    }
...
#[derive(Serialize)]
struct AwacAntMazeConfig {
    args: Args,
    trainer_config: TrainerConfig,
    agent_config: AwacConfig<Mlp, Mlp3>,
}

impl AwacAntMazeConfig {
...
fn create_trainer(config: &AwacAntMazeConfig) -> Trainer {
    log::info!("Create trainer");
    Trainer::build(config.trainer_config.clone())
}

fn create_agent<E, R>(config: &AwacAntMazeConfig) -> Box<dyn Agent<E, R>>
where
    E: Env + 'static,
    E::Obs: Into<Tensor>,
...
fn create_recorder<E, R>(config: &AwacAntMazeConfig) -> Result<Box<dyn Recorder<E, R>>>
where
    E: Env + 'static,
    R: ReplayBufferBase + 'static,
{
...
fn train<T>(config: AwacAntMazeConfig, dataset: MinariDataset, mut converter: T) -> Result<()>
where
...
fn eval<T>(config: AwacAntMazeConfig, dataset: MinariDataset, converter: T) -> Result<()>
where
    T: MinariConverter + 'static,
    T::Obs: std::fmt::Debug + Into<Tensor>,
    T::Act: std::fmt::Debug + From<Tensor> + Into<Tensor>,
...
fn main() -> Result<()> {
    ...
    let config = AwacAntMazeConfig::new(args.clone());
    let dataset = MinariDataset::load_dataset(args.dataset_name(), true)?;
    let converter = AntMazeConverter::new(AntMazeConverterConfig {}, &dataset)?;
