...
use border_core::{
    ...
    Agent, Configurable, DefaultEvaluator, Evaluator as _, ReplayBufferBase, StepProcessor,
...
fn train(
    ...
    let mut trainer =
        Trainer::<Env>::build(config.trainer_config.clone(), config.env_config.clone());
