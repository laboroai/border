rm -f log_dqn_atari_async_tch.txt
sh run_detach.sh "
    ...
    cargo run --release --example dqn_atari_async_tch --features=tch,border-async-trainer \
    -- ${1} --mlflow --n-actors 6 > \
    $HOME/border/docker/amd64_headless/log_dqn_atari_async_tch_${1}.txt 2>&1"
